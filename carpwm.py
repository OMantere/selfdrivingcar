...
class CarControl(object):
    def __init__(self, esc_pin, steerpin, verbose=True):
    ...
    def set_steering_angle(self, value):
        ...
        self.steering.unit_adjust(1.0 - value)

    def set_throttle(self, value):
        if self.verbose:
            print("Set throttle to %.2f" % value)
        self.throttle.unit_adjust(1.0 - value)
...
class KeyboardControl(object):
    ...
    def start(self):
        ...
        try:
            while 1:
                c = screen.getch()
                if c == ord('a') and steering < 1.0:
                    steering -= 0.1
                elif c == ord('d') and steering > 0.0:
                    steering += 0.1
                elif c == ord('w'):
                    throttle += 0.01
                elif c == ord('s'):
